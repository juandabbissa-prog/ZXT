...
describe('keyword mapper', () => {
  it('maps Prisma relation rows to a domain record without relation payloads', () => {
    const row = {
      ...
      archivedAt: null,
      deletedAt: null,
      roles: [{ keywordId: 'keyword-1', role: 'DISCOVERY', createdAt: new Date() }],
      tags: [{ keywordId: 'keyword-1', tagId: 'tag-1', createdAt: new Date() }],
    } as unknown as Parameters<typeof toKeywordRecord>[0];
