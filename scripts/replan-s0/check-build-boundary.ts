...
for (let index = 0; index < paths.length; index++) {
  const item = JSON.parse(await Bun.file(`${root}/${paths[index]}`).text());
  if (item.name !== allowed[index] || item.scripts?.build !== 'tsc --noEmit')
    throw new Error(`Unsafe alternative build boundary: ${allowed[index]}`);
}
...
for (const workspace of allowed) {
  const child = Bun.spawn(['bun', '--filter', workspace, 'build'], {
    cwd: root,
    stdout: 'inherit',
    stderr: 'inherit',
  });
  const code = await child.exited;
  if (code !== 0) throw new Error(`Alternative build failed: ${workspace}`);
}
process.stdout.write(
  'ALTERNATIVE_STATIC_VALIDATION_PASS included shared/sdk/crawler/workers only\n',
);
