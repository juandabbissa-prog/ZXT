...
async function scan(path: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (
      !/\.(?:ts|tsx|js|mjs|json|md|ya?ml|sh|ps1|example|Dockerfile)$/i.test(entry.name) &&
    ...
    lines.forEach((line, index) =>
      rules.forEach(([id, pattern]) => {
        if (pattern.test(line))
          violations.push(`${relative(root, full)}|${id}|${index + 1}|REDACTED`);
      }),
    );
  }
}
for (const path of roots) {
  ...
  try {
    if ((await stat(full)).isDirectory()) await scan(full);
  } catch (error) {
...
process.stdout.write(
  'SECRET_STATIC_SCAN_PASS controlled paths only; this does not assert NO_SECRET_EXISTS\n',
);
